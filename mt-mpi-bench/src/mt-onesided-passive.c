...
#pragma omp parallel for
    ...
    if (rank < half_rank) {
        ...
        for (int i = 0; i < ITERATIONS; i++) {
            MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank + half_rank, 0, local_window);
...
#pragma omp parallel for
            ...
            MPI_Win_unlock(rank + half_rank, local_window);
        }
    ...
    if (half_rank == rank) {
        ...
        for (int i = 0; i < local_data_count; i++) {
            if (local_data[i] != (double) i) {
                fprintf(stderr,
                        "Error: value at index %d should be %f value is %f\n",
                        i, (double) i, local_data[i]);
                exit(-1);
            }
        }
