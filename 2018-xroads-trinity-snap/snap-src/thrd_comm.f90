...
MODULE thrd_comm_module
  ...
  PUBLIC :: assign_thrd_set, no_op_lock_control, sweep_recv_bdry,      &
    sweep_send_bdry, corsweep_recv_bdry, corsweep_test_pick,           &
    corsweep_send_bdry
...
!_______________________________________________________________________
  ...
  SUBROUTINE corsweep_recv_bdry ( jdim, kdim, ncor, ng, ic_op, gc,     &
    grp_act, callrecv, mtag, reqr, szreq, nang, ichunk, ny, nz, jb_in, &
    kb_in )
...
!-----------------------------------------------------------------------
    ...
    INTEGER(i_knd), DIMENSION(ncor), INTENT(INOUT) :: mtag
...
!_______________________________________________________________________
  ...
  END SUBROUTINE corsweep_recv_bdry


  SUBROUTINE corsweep_test_pick ( ncor, corner_loop_order, yzstg, nstg,&
    nulreq, prev_c, g_op, gc, ng, grp_act, reqr, szreqr, done, reqs,   &
    szreqs, cor )
...
!-----------------------------------------------------------------------
    ...
    INTEGER(i_knd), INTENT(IN) :: ncor, nstg, nulreq, prev_c(2),       &
      szreqr, szreqs, ng
...
!_______________________________________________________________________
      ...
      recv_cor_loop: DO
        ...
        DO j = 1, ncor
          ...
          IF ( done(i) ) CYCLE
          IF ( reqr(i)==nulreq .AND. reqr(ncor+i)==nulreq ) then
            g = grp_act(gc(i))
            tcor(i) = nstg - yzstg(i) - g_op(i)
          END IF
        END DO
!_______________________________________________________________________
!
...
!_______________________________________________________________________
        ...
        IF ( ANY( tcor >= 0 ) ) THEN
          cor = MAXLOC( tcor, 1 )
!          IF ( mxadv>0 .AND. prev_c(1)>0 .AND. prev_c(2)<mxadv .AND.   &
!               tcor(prev_c(1))>=0 ) cor = prev_c(1)
          EXIT recv_cor_loop
        ELSE
          skip_cor = 0
!          CALL waitsome ( reqr, indx, szreqr, outcount )
        END IF
...
!_______________________________________________________________________
  ...
  END SUBROUTINE corsweep_test_pick


  SUBROUTINE corsweep_send_bdry ( jd, kd, nang, ichunk, ny, nz, ncor,  &
...
!-----------------------------------------------------------------------
    ...
    INTEGER(i_knd), INTENT(IN) :: jd, kd, nang, ichunk, ny, nz, ncor,  &
      mtag
...
!_______________________________________________________________________
  ...
  END SUBROUTINE corsweep_send_bdry
