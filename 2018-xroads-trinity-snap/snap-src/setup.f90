...
MODULE setup_module
  ...
  USE control_module, ONLY: epsi, iitm, oitm, timedep, tf, nsteps, dt, &
    ...
    nstg, last_oct, corner_sch, cor_swp, yzstg, corner_loop_order,     &
    control_allocate
...
!_______________________________________________________________________
    ...
    IF ( cor_swp == 1 ) THEN
      ...
      nops = ng*ncor*2*nc
      nstg = 2*nc + npey + npez - 2
      swp_typ=0

      DO i = 1, ncor
        IF ( i==1 .OR. i==3 ) THEN
          yzstg(i) = npey - (yproc + 1)
        ELSE
          yzstg(i) = yproc
        END IF
        IF ( i <= 2 ) THEN
...
!_______________________________________________________________________
    ...
    IF ( ierr /= 0 ) THEN
      flg = 2
      error = '***ERROR: CONTROL_ALLOC: Allocation error of ' //       &
              'control arrays'
      RETURN
    END IF
