...
MODULE solvar_module
  ...
  USE control_module, ONLY: timedep, angcpy, ncor, cor_swp
...
! jb_out(nang,ichunk,nz,ncor,2*nc,ng) - y-dir boundary flux out to comm
! kb_in(nang,ichunk,ny,ncor,ng)  - z-dir boundary flux in from comm
! kb_out(nang,ichunk,ny,ncor,2*nc,ng) - z-dir boundary flux out to comm
!
...
!_______________________________________________________________________
  ...
  REAL(r_knd), ALLOCATABLE, DIMENSION(:,:,:,:,:) :: q2grpm, fluxm,     &
    s_xs, psii, jb_in, kb_in
    !s_xs, psii, jb_in, jb_out, kb_in, kb_out
  REAL(r_knd), ALLOCATABLE, DIMENSION(:,:,:,:,:,:) :: jb_out, kb_out
...
!_______________________________________________________________________
    ...
    ALLOCATE( psii(nang,ny,nz,ncor,ng), psij(nang,ichunk,nz,ng),       &
...
!_______________________________________________________________________
    ...
    ALLOCATE( jb_in(nang,ichunk,nz,ncor,ng),                           &
      jb_out(nang,ichunk,nz,ncor,2*nc,ng), kb_in(nang,ichunk,ny,ncor,ng),   &
      kb_out(nang,ichunk,ny,ncor,2*nc,ng), STAT=ierr )
    IF ( ierr /= 0 ) RETURN
