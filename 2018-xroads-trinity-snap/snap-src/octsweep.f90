...
MODULE octsweep_module
  ...
  USE control_module, ONLY: timedep, swp_typ, cor_swp
...
!_______________________________________________________________________
!
!   For cor_swp=0, set loops over all the chunks in the x-domain,
!   including the octant pair. Otherwise, corsweep scheduling handles
!   sweeping all chunks.
!_______________________________________________________________________
...
!_______________________________________________________________________
          ...
          IF ( ndimen == 1 ) THEN
            ...
            CALL dim1_sweep ( id, d1, d2, d3, d4, oct, g,              &
              psii(:,1,1,1,g), qtot(:,:,1,1,ich,g), ec(:,:,oct),       &
              vdelt(g), ptr_in(:,:,:,:,d5,d6), ptr_out(:,:,:,:,d5,d6), &
              dinv(:,:,1,1,ich,g), flux0(:,1,1,g), fluxm(:,:,1,1,g),   &
              wmu, flkx(:,1,1,g), t_xs(:,1,1,g), fmin(g), fmax(g) )
  
          ELSE

            CALL dim3_sweep ( ich, id, d1, d2, d3, d4, jd, kd, oct, g, &
              t, iop, reqs, szreq, psii(:,:,:,1,g), psij(:,:,:,g),     &
              psik(:,:,:,g), qtot(:,:,:,:,ich,g), ec(:,:,oct),         &
              vdelt(g), ptr_in(:,:,:,:,d5,d6), ptr_out(:,:,:,:,d5,d6), &
              dinv(:,:,:,:,ich,g), flux0(:,:,:,g), fluxm(:,:,:,:,g),   &
              jb_in(:,:,:,1,g), jb_out(:,:,:,1,iop,g), kb_in(:,:,:,1,g),   &
              kb_out(:,:,:,1,iop,g), wmu, weta, wxi, flkx(:,:,:,g),        &
...
!_______________________________________________________________________
      ...
      ELSE
        ...
        CALL mkba_sweep ( d1, d2, d3, d4, d5, jd, kd, g, t, reqs,      &
          szreq, psii(:,:,:,1,g), psij(:,:,:,g), psik(:,:,:,g),        &
          qtot(:,:,:,:,:,g), ec, vdelt(g), ptr_in(:,:,:,:,:,d6),       &
          ptr_out(:,:,:,:,:,d6),  dinv(:,:,:,:,:,g), flux0(:,:,:,g),   &
          fluxm(:,:,:,:,g), jb_in(:,:,:,1,g), jb_out(:,:,:,1,iop,g),       &
          kb_in(:,:,:,1,g), kb_out(:,:,:,1,iop,g), wmu, weta, wxi,         &
...
!_______________________________________________________________________
!
!   Same call to dim3_sweep, but use correct corner setting. Only reach
!   here for dim3_sweep call.
!_______________________________________________________________________
    ...
    ELSE
      ...
      CALL dim3_sweep ( ich, id, d1, d2, d3, d4, jd, kd, oct, g, t,    &
        iop, reqs, szreq, psii(:,:,:,cor,g), psij(:,:,:,g),            &
        psik(:,:,:,g), qtot(:,:,:,:,ich,g), ec(:,:,oct), vdelt(g),     &
        ptr_in(:,:,:,:,d5,d6), ptr_out(:,:,:,:,d5,d6),                 &
        dinv(:,:,:,:,ich,g), flux0(:,:,:,g), fluxm(:,:,:,:,g),         &
        jb_in(:,:,:,cor,g), jb_out(:,:,:,cor,iop,g), kb_in(:,:,:,cor,g),   &
        kb_out(:,:,:,cor,iop,g), wmu, weta, wxi, flkx(:,:,:,g),            &
        flky(:,:,:,g), flkz(:,:,:,g), t_xs(:,:,:,g), fmin(g), fmax(g) )
